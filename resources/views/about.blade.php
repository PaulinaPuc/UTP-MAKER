@extends('layouts.main')
@section('content')
<!--------------- blog-tittle-section---------------->
    <section class="blog about-blog">
        <div class="container">
            <div class="blog-bradcrum">
                <span><a href="/">inicio</a></span>
                <span class="devider">/</span>
                <span><a href="/about">Conocenos</a></span>
            </div>
            <div class="blog-heading about-heading">
                <h1 class="heading text-3xl ">CONOCENOS</h1>
            </div>
        </div>
    </section>
    <!--------------- blog-tittle-section-end---------------->

    <!--------------- about-section---------------->
    <section class="about">
        <div class="container">
            <div class="about-section">
                <div class="row align-items-center gy-5">
                    <div class="col-lg-6">
                        <div class="about-img" data-aos="fade-right">
                            <img src="assets/images/homepage-one/about/about-img-1.webp" alt="img">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="about-content" data-aos="fade-up">
                            <h3 class="about-title">Sobre Nosotros</h3>
                            <p class="about-info">
                                somos una empresa dedicada a la venta de objetos personalizados con tecnología de impresión 3D y sublimación.
                             entre mas cosas para ofrcer calidad de productos con la facilidad de compra y venta</p
                            <div class="about-list">
                                <ul>
                                    <li>
                                        <span>
                                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <circle cx="12.5" cy="12.5" r="12.5" fill="#AE1C9A" />
                                                <path
                                                    d="M10.1691 13.2566C10.5172 12.8649 10.8498 12.4803 11.198 12.1029C12.7761 10.3864 14.4973 8.80535 16.4699 7.47353C16.6749 7.33465 16.8876 7.20289 17.1042 7.0747C17.1739 7.03552 17.2628 7.00347 17.344 7.00347C17.7888 6.99635 18.2337 6.99991 18.6746 6.99991C18.8138 6.99991 18.926 7.04265 18.9763 7.16728C19.0266 7.28836 18.9879 7.39163 18.8835 7.48065C17.0772 8.99765 15.588 10.7639 14.1724 12.5872C12.8689 14.2644 11.6621 16.0022 10.5288 17.7863C10.4901 17.8504 10.4398 17.918 10.3741 17.9572C10.2348 18.0462 10.0763 17.9964 9.97183 17.8432C9.79777 17.5868 9.63532 17.3233 9.44966 17.074C8.36278 15.6318 7.26817 14.1896 6.17742 12.751C6.13488 12.6976 6.08846 12.6441 6.04978 12.5872C5.97243 12.4732 5.97629 12.3486 6.07686 12.256C6.36695 11.9853 6.66478 11.7147 6.96261 11.4476C7.07864 11.3444 7.20242 11.3515 7.35713 11.4476C7.83675 11.7539 8.31637 12.0637 8.79212 12.3699C9.24853 12.6655 9.70495 12.9575 10.1691 13.2566Z"
                                                    fill="white" />
                                            </svg>

                                        </span>
                                        <p>objetos personalizados </p>
                                    </li>
                                    <li>
                                        <span>
                                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <circle cx="12.5" cy="12.5" r="12.5" fill="#AE1C9A" />
                                                <path
                                                    d="M10.1691 13.2566C10.5172 12.8649 10.8498 12.4803 11.198 12.1029C12.7761 10.3864 14.4973 8.80535 16.4699 7.47353C16.6749 7.33465 16.8876 7.20289 17.1042 7.0747C17.1739 7.03552 17.2628 7.00347 17.344 7.00347C17.7888 6.99635 18.2337 6.99991 18.6746 6.99991C18.8138 6.99991 18.926 7.04265 18.9763 7.16728C19.0266 7.28836 18.9879 7.39163 18.8835 7.48065C17.0772 8.99765 15.588 10.7639 14.1724 12.5872C12.8689 14.2644 11.6621 16.0022 10.5288 17.7863C10.4901 17.8504 10.4398 17.918 10.3741 17.9572C10.2348 18.0462 10.0763 17.9964 9.97183 17.8432C9.79777 17.5868 9.63532 17.3233 9.44966 17.074C8.36278 15.6318 7.26817 14.1896 6.17742 12.751C6.13488 12.6976 6.08846 12.6441 6.04978 12.5872C5.97243 12.4732 5.97629 12.3486 6.07686 12.256C6.36695 11.9853 6.66478 11.7147 6.96261 11.4476C7.07864 11.3444 7.20242 11.3515 7.35713 11.4476C7.83675 11.7539 8.31637 12.0637 8.79212 12.3699C9.24853 12.6655 9.70495 12.9575 10.1691 13.2566Z"
                                                    fill="white" />
                                            </svg>

                                        </span>
                                        <p>facilidad de compra y venta</p>
                                    </li>
                                    <li>
                                        <span>
                                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <circle cx="12.5" cy="12.5" r="12.5" fill="#AE1C9A" />
                                                <path
                                                    d="M10.1691 13.2566C10.5172 12.8649 10.8498 12.4803 11.198 12.1029C12.7761 10.3864 14.4973 8.80535 16.4699 7.47353C16.6749 7.33465 16.8876 7.20289 17.1042 7.0747C17.1739 7.03552 17.2628 7.00347 17.344 7.00347C17.7888 6.99635 18.2337 6.99991 18.6746 6.99991C18.8138 6.99991 18.926 7.04265 18.9763 7.16728C19.0266 7.28836 18.9879 7.39163 18.8835 7.48065C17.0772 8.99765 15.588 10.7639 14.1724 12.5872C12.8689 14.2644 11.6621 16.0022 10.5288 17.7863C10.4901 17.8504 10.4398 17.918 10.3741 17.9572C10.2348 18.0462 10.0763 17.9964 9.97183 17.8432C9.79777 17.5868 9.63532 17.3233 9.44966 17.074C8.36278 15.6318 7.26817 14.1896 6.17742 12.751C6.13488 12.6976 6.08846 12.6441 6.04978 12.5872C5.97243 12.4732 5.97629 12.3486 6.07686 12.256C6.36695 11.9853 6.66478 11.7147 6.96261 11.4476C7.07864 11.3444 7.20242 11.3515 7.35713 11.4476C7.83675 11.7539 8.31637 12.0637 8.79212 12.3699C9.24853 12.6655 9.70495 12.9575 10.1691 13.2566Z"
                                                    fill="white" />
                                            </svg>

                                        </span>
                                        <p>rapidez en la entrega</p>
                                    </li>
                                </ul>
                            </div>
                            <a href="/contact-us" class="shop-btn">
                                Contactanos
                                <span>
                                    <svg width="8" height="14" viewBox="0 0 8 14" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <rect x="1.45312" y="0.914062" width="9.25346" height="2.05632"
                                            transform="rotate(45 1.45312 0.914062)" fill="white" />
                                        <rect x="8" y="7.45703" width="9.25346" height="2.05632"
                                            transform="rotate(135 8 7.45703)" fill="white" />
                                    </svg>

                                </span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--------------- about-section-end---------------->

    <!--------------- about-service---------------->
    <section class="about-service product ">
        <div class="container">
            <div class="about-service-section">
                <div class="about-wrapper">
                    <div class="wrapper-img">
                        <span>
                            <svg width="104" height="104" viewBox="0 0 104 104" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="52" cy="52" r="52" fill="#AE1C9A" />
                                <path
                                    d="M33.2764 39.0627C33.2764 39.3893 33.2764 39.6479 33.2764 39.9064C33.2764 47.772 33.2764 55.6376 33.2764 63.5033C33.2764 64.7008 33.3443 64.7688 34.5392 64.7824C37.4858 64.7824 40.446 64.7824 43.3926 64.7824C43.6778 64.7824 43.9901 64.8097 44.2345 64.9321C44.3703 65.0002 44.4789 65.3132 44.4518 65.4765C44.411 65.667 44.2209 65.9119 44.0308 65.98C43.8 66.0752 43.5148 66.048 43.2432 66.0616C40.2966 66.0616 37.3365 66.0616 34.3899 66.0616C32.6925 66.0616 32 65.3812 32 63.7074C32 53.923 32 44.1386 32 34.3542C32 32.6532 32.6654 32 34.3763 32C46.4207 32 58.4787 32 70.5231 32C72.2068 32 72.8722 32.6668 72.8722 34.3815C72.8722 44.1386 72.8722 53.8958 72.8722 63.6529C72.8722 65.3812 72.2068 66.048 70.4416 66.048C68.8936 66.048 67.3592 66.0616 65.8112 66.048C65.1594 66.0344 64.7521 65.6262 65.0236 65.2043C65.173 64.973 65.5668 64.796 65.852 64.796C67.4407 64.7552 69.043 64.7688 70.6453 64.7824C71.3106 64.7824 71.6094 64.5239 71.5958 63.8299C71.5822 55.6921 71.5822 47.5679 71.5822 39.4301C71.5822 39.3213 71.5686 39.226 71.5551 39.0491C58.8181 39.0627 46.0948 39.0627 33.2764 39.0627ZM33.2764 37.6475C46.1084 37.6475 58.8181 37.6475 71.5822 37.6475C71.5822 36.5724 71.5822 35.5654 71.5822 34.5448C71.5822 33.32 71.5415 33.2792 70.3194 33.2792C58.3836 33.2792 46.4478 33.2792 34.5121 33.2792C34.3491 33.2792 34.1998 33.2792 34.0368 33.2792C33.5616 33.2792 33.2764 33.4969 33.2764 34.0004C33.2764 35.198 33.2764 36.3955 33.2764 37.6475Z"
                                    fill="white" />
                                <path d="M42 46H62V49H42V46Z" fill="white" />
                                <path d="M42 54H55V57H42V54Z" fill="white" />
                            </svg>
                        </span>
                    </div>
                    <div class="wrapper-info">
                        <h5 class="wrapper-details about-details">Misión</h5>
                        <p>Desarrollar soluciones innovadoras y de alta calidad a través de nuestros proyectos y emprendimientos universitarios, abarcando áreas como fotografía profesional, tecnología láser, robótica, sublimación, tazas e impresión 3D, transformando la creatividad y el talento en productos reales.</p>
                    </div>
                </div>
                <div class="seperator">
                </div>
                <div class="about-wrapper">
                    <div class="wrapper-img">
                        <span>
                            <svg width="104" height="104" viewBox="0 0 104 104" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="52" cy="52" r="52" fill="#AE1C9A" />
                                <path
                                    d="M62.9706 54.2125C64.9571 52.371 66.8939 50.6291 68.7645 48.8208C70.4862 47.1783 73.1183 47.3111 74.4592 49.1857C75.32 50.3968 75.3365 52.4042 74.3764 53.5821C71.5953 56.9831 68.8308 60.3841 65.9503 63.6855C64.3942 65.4606 62.2422 66.1076 59.9081 66.1242C56.051 66.1408 52.1939 66.174 48.3202 66.1076C47.0124 66.0911 46.1019 66.6717 45.3073 67.5676C45.1584 67.7335 45.1252 68.0653 45.1418 68.2975C45.2411 69.2432 44.9763 70.0561 44.3141 70.7363C42.9235 72.1299 41.5164 73.4902 40.1425 74.8838C39.5465 75.4811 39.0168 75.4976 38.4208 74.8838C35.1265 71.5326 31.8157 68.2146 28.5215 64.8634C27.8262 64.1666 27.8262 63.785 28.5215 63.0717C29.8789 61.6947 31.2032 60.2845 32.6269 58.9739C33.1069 58.526 33.8188 58.2108 34.4809 58.0946C34.9941 57.9951 35.259 57.8458 35.5404 57.431C38.007 53.6817 41.5827 52.1056 45.9364 52.2715C47.1945 52.3213 48.4361 52.736 49.6776 52.9683C50.0915 53.0512 50.5219 53.1176 50.9523 53.1176C53.8327 53.1342 56.7131 53.1176 59.5935 53.1176C61.4311 53.1508 61.8118 53.2835 62.9706 54.2125ZM44.0327 66.2735C45.4232 64.3823 47.3766 64.1998 49.479 64.2496C52.9388 64.3159 56.3986 64.2827 59.8584 64.2661C61.8118 64.2661 63.55 63.6855 64.8412 62.1592C67.4733 59.0403 70.0723 55.9047 72.6713 52.7692C73.499 51.7738 73.499 50.712 72.7044 49.9821C71.9264 49.2853 70.8172 49.4014 69.9399 50.2807C68.1686 52.1056 66.3807 53.9139 64.6426 55.772C64.328 56.1038 64.0963 56.6513 64.0632 57.1158C63.8811 58.8578 62.7223 60.3343 60.951 60.4338C58.3023 60.5997 55.6371 60.55 52.9719 60.55C52.3925 60.55 51.9455 60.2347 51.9952 59.5711C52.0283 58.9241 52.4918 58.7085 53.0712 58.7085C53.8989 58.7085 54.7266 58.7085 55.5543 58.7085C57.1104 58.7085 58.6831 58.7416 60.2392 58.6919C61.6628 58.6421 62.5071 57.5472 62.1429 56.3029C61.8946 55.4568 61.1662 55.0089 59.9577 55.0089C56.8621 54.9923 53.7665 55.0089 50.6874 54.9923C50.2074 54.9923 49.6942 54.9425 49.2472 54.7766C47.1614 53.9637 45.0094 53.8807 42.8573 54.3784C40.1424 55.0089 38.0732 56.5352 36.7654 59.0071C39.1989 61.4458 41.5661 63.8016 44.0327 66.2735ZM30.1603 63.9841C33.3056 67.0533 36.335 70.0063 39.4803 73.0589C40.5894 71.9142 41.8144 70.6699 43.0063 69.4091C43.4367 68.9611 43.4533 68.4634 43.0063 68.0155C40.4404 65.4275 37.8745 62.8394 35.2921 60.2679C34.8451 59.82 34.3651 59.82 33.9181 60.2513C32.7097 61.4624 31.5012 62.6735 30.1603 63.9841Z"
                                    fill="white" />
                            </svg>
                        </span>
                    </div>
                    <div class="wrapper-info">
                        <h5 class="wrapper-details about-details">Visión</h5>
                        <p>Consolidarnos como un referente de innovación estudiantil y espíritu empresarial, destacando en el mercado con productos personalizados, tecnológicos y de vanguardia que impulsen el talento universitario y el emprendimiento.</p>
                    </div>
                </div>
                <div class="seperator">
                </div>
                <div class="about-wrapper">
                    <div class="wrapper-img">
                        <span>
                            <svg width="104" height="104" viewBox="0 0 104 104" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <circle cx="52" cy="52" r="52" fill="#AE1C9A" />
                                <path
                                    d="M74.5284 66.3763C73.9211 68.1456 72.7912 69.4195 70.8563 69.7733C68.6954 70.1696 66.7181 69.1081 65.8425 67.0274C65.6024 66.4471 65.3058 66.2914 64.7126 66.3056C59.9671 66.3197 55.2075 66.3197 50.4621 66.3056C49.9112 66.3056 49.6288 66.4613 49.4169 66.985C48.6684 68.8392 47.2137 69.8441 45.2505 69.8582C43.2309 69.8724 41.7479 68.8675 40.9852 66.9708C40.7593 66.4329 40.4768 66.2914 39.9401 66.3056C37.9063 66.3339 35.8726 66.2914 33.8388 66.3339C32.8925 66.348 32.5818 65.9376 32.5818 65.0458C32.5818 63.3898 33.1609 62.0735 34.5591 61.1393C34.785 60.9978 35.0534 60.7005 35.0534 60.4741C35.1099 59.4267 35.0816 58.3792 35.0816 57.2186C33.2456 57.2186 31.5225 57.2186 29.7995 57.2186C28.9238 57.2186 28.034 57.2186 27.1584 57.2186C26.5793 57.1903 26.0003 57.0771 26.0003 56.3552C26.0003 55.6333 26.5652 55.506 27.1443 55.506C29.3899 55.506 31.6214 55.506 33.867 55.506C34.2342 55.506 34.6014 55.506 35.0251 55.506C35.0251 54.9398 35.0251 54.4727 35.0251 53.8499C34.0506 53.8499 33.0902 53.8358 32.1157 53.8499C31.5225 53.8641 30.9576 53.7508 30.9717 53.029C30.9717 52.3213 31.5225 52.1797 32.1157 52.1939C33.062 52.208 34.0083 52.1939 35.011 52.1939C35.011 51.656 35.011 51.1889 35.011 50.5945C34.6862 50.5803 34.3331 50.552 33.98 50.552C31.7061 50.552 29.4323 50.5379 27.1584 50.552C26.5652 50.552 26.0003 50.4529 26.0003 49.7311C25.9861 49.0092 26.5228 48.8818 27.1301 48.896C29.1639 48.9101 31.1977 48.896 33.2315 48.896C33.3303 48.7827 33.4292 48.6695 33.5422 48.5563C33.3021 48.4572 33.0196 48.4289 32.836 48.259C30.1667 45.9095 29.3052 42.3143 30.6045 39.0872C31.9039 35.8459 34.9969 33.8784 38.556 34.0058C41.8326 34.1191 44.8551 36.4262 45.8578 39.7099C46.0697 40.4177 46.3663 40.5875 47.0442 40.5875C50.7445 40.5592 54.4449 40.5734 58.1452 40.5734C61.5207 40.5734 63.2296 42.2718 63.2438 45.6264C63.2438 46.6738 63.2438 47.7212 63.2438 48.8818C65.3199 48.8818 67.3113 48.9384 69.3027 48.8677C72.198 48.7544 74.1612 50.0425 75.856 52.3637C77.5932 54.7699 78.4971 57.2186 78.1722 60.191C78.0028 61.7338 78.144 63.319 78.144 64.876C78.144 66.1216 78.0028 66.2631 76.7175 66.2631C76.0679 66.2631 75.4323 66.2773 74.7826 66.2773C74.6979 66.3056 74.6273 66.348 74.5284 66.3763ZM36.7906 60.4458C45.081 60.4458 53.2867 60.4458 61.4783 60.4458C61.5207 60.2901 61.5631 60.191 61.5772 60.0919C61.5772 55.053 61.6055 50.0283 61.5772 44.9894C61.5631 43.3051 60.4191 42.2577 58.6678 42.2435C54.798 42.2152 50.9281 42.2435 47.0583 42.2152C46.3663 42.2152 46.2815 42.5125 46.1403 43.1069C45.9002 44.1543 45.6318 45.2301 45.1093 46.1642C44.5585 47.1409 43.7252 47.9477 42.9625 48.8818C44.3607 48.8818 45.7307 48.896 47.1148 48.8818C47.708 48.8818 48.2871 48.9526 48.3012 49.6745C48.3294 50.4529 47.7362 50.552 47.0866 50.552C44.6432 50.5379 42.1999 50.552 39.7565 50.552C38.782 50.552 37.8075 50.552 36.8329 50.552C36.8329 51.1606 36.8329 51.6419 36.8329 52.1939C37.2425 52.1939 37.5815 52.1939 37.9063 52.1939C39.361 52.1939 40.8299 52.1939 42.2846 52.1939C42.8637 52.1939 43.3156 52.3779 43.3156 53.0431C43.3156 53.7225 42.8213 53.8499 42.2563 53.8499C41.7761 53.8499 41.3101 53.8499 40.8299 53.8499C39.4882 53.8499 38.1464 53.8499 36.8047 53.8499Z"
                                    fill="white" />
                            </svg>
                        </span>
                    </div>
                    <div class="wrapper-info">
                        <h5 class="wrapper-details about-details">Valores</h5>
                        <p>Innovación, trabajo en equipo, calidad, compromiso y creatividad. Fomentamos la colaboración multidisciplinaria para ofrecer productos excepcionales y de alto valor agregado en cada uno de nuestros servicios.</p>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!--------------- about-promotion-section---------------->
    <div class="about-promotion">
        <a href="assets/images/homepage-one/about/tareas.mp4" target="_blank" class="about-btn">
            <span>
                <svg width="34" height="38" viewBox="0 0 34 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M5.19276 0.628906C6.04182 0.925379 6.95574 1.10689 7.72983 1.53849C15.5883 5.91299 23.4346 10.3097 31.2626 14.7386C34.8453 16.7655 34.8595 21.3861 31.2829 23.413C23.4569 27.846 15.6126 32.2467 7.75617 36.6252C4.10052 38.6622 0.0780744 36.3267 0.0618631 32.1478C0.0294404 23.4452 0.0395725 14.7426 0.0578102 6.04005C0.0659159 2.98657 2.26255 0.751933 5.19276 0.628906Z"
                        fill="#AE1C9A" />
                </svg>
            </span>
        </a>
        <video src="assets/images/homepage-one/about/tareas.mp4" autoplay loop muted></video>
    </div>
    <!--------------- about-promotion-end---------------->

    <!--------------- about-slider-section---------------->
    <section class="about-feedback product">
        <div class="container p-0">
            <div class="position-relative px-5">
                <div class="swiper about-swiper">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide testimonial-wrapper">
                            <div class="blockquote">
                                <span>
                                    <svg width="38" height="30" viewBox="0 0 38 30" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.82644 11.9446C8.29006 9.03034 11.9328 5.91742 14.7808 5.85119C14.9795 5.85119 15.1782 5.78496 15.3107 5.65249C15.4431 5.58626 15.5756 5.52003 15.6418 5.32133C16.6353 3.46683 16.1055 2.00972 14.4497 0.817536C12.5289 -0.573341 9.48225 0.817536 7.9589 2.07595C4.11743 5.2551 0.20973 10.7523 0.408427 15.9847C-0.253896 19.4951 -0.121431 23.2703 0.872052 26.3832C1.53437 28.3702 3.45511 29.3636 5.44208 29.4961C7.42905 29.6287 11.5354 30.2247 13.3237 29.0326C15.112 27.8403 15.2445 25.5222 15.4431 23.5353C15.6418 21.3496 16.2379 17.2431 14.3834 15.5211C12.5289 13.8653 7.23035 15.6536 7.82644 11.9446Z"
                                            fill="#f6f6f6" />
                                        <path
                                            d="M29.683 11.9446C30.1466 9.03034 33.7893 5.91742 36.6374 5.85119C36.8361 5.85119 37.0348 5.78496 37.1673 5.65249C37.2998 5.58626 37.4322 5.52003 37.4985 5.32133C38.492 3.46683 37.9622 2.00972 36.3064 0.817536C34.3856 -0.573341 31.3389 0.817536 29.8155 2.07595C25.974 5.2551 22.0663 10.7524 22.265 15.9847C21.6027 19.4951 21.7351 23.2703 22.7285 26.3832C23.3908 28.3702 25.3116 29.3636 27.2987 29.4961C29.2856 29.6287 33.392 30.2247 35.1803 29.0326C36.9685 27.8403 37.101 25.5222 37.2997 23.5353C37.4984 21.3496 38.0945 17.2431 36.24 15.5211C34.3855 13.8653 29.0207 15.6536 29.683 11.9446Z"
                                            fill="#f6f6f6" />
                                    </svg>
                                </span>
                            </div>
                            <p class="testimonial-details">Excelente pagina, con un diseño muy atractivo y facil de usar,
                                <span class="testimonial-inner-text">
                                    me ha encantado la experiencia y toda la informcaion,
                                    sin duda volveré, para informarme mas.
                                </span>
                            </p>
                            <div class="ratings">
                                <span>
                                    <svg width="75" height="15" viewBox="0 0 75 15" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M22.5 0L24.1839 5.18237H29.6329L25.2245 8.38525L26.9084 13.5676L22.5 10.3647L18.0916 13.5676L19.7755 8.38525L15.3671 5.18237H20.8161L22.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M37.5 0L39.1839 5.18237H44.6329L40.2245 8.38525L41.9084 13.5676L37.5 10.3647L33.0916 13.5676L34.7755 8.38525L30.3671 5.18237H35.8161L37.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M52.5 0L54.1839 5.18237H59.6329L55.2245 8.38525L56.9084 13.5676L52.5 10.3647L48.0916 13.5676L49.7755 8.38525L45.3671 5.18237H50.8161L52.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M67.5 0L69.1839 5.18237H74.6329L70.2245 8.38525L71.9084 13.5676L67.5 10.3647L63.0916 13.5676L64.7755 8.38525L60.3671 5.18237H65.8161L67.5 0Z"
                                            fill="#FFA800" />
                                    </svg>
                                </span>
                            </div>
                            <div class="divider"></div>
                            <div class="testimonial-info">
                                <div class="testimonial-img">
                                    <img src="assets/images/homepage-one/about/testimonial-img-1.webp" alt="img">
                                </div>
                                <div class="testimonial-info-details">
                                    <h5 class="testimonial-name">Juan Perez</h5>
                                    <p class="testimonial-title">Director ejecutivo de <span class="title-inner">DesginCraft</span></p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide testimonial-wrapper">
                            <div class="blockquote">
                                <span>
                                    <svg width="38" height="30" viewBox="0 0 38 30" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.82644 11.9446C8.29006 9.03034 11.9328 5.91742 14.7808 5.85119C14.9795 5.85119 15.1782 5.78496 15.3107 5.65249C15.4431 5.58626 15.5756 5.52003 15.6418 5.32133C16.6353 3.46683 16.1055 2.00972 14.4497 0.817536C12.5289 -0.573341 9.48225 0.817536 7.9589 2.07595C4.11743 5.2551 0.20973 10.7523 0.408427 15.9847C-0.253896 19.4951 -0.121431 23.2703 0.872052 26.3832C1.53437 28.3702 3.45511 29.3636 5.44208 29.4961C7.42905 29.6287 11.5354 30.2247 13.3237 29.0326C15.112 27.8403 15.2445 25.5222 15.4431 23.5353C15.6418 21.3496 16.2379 17.2431 14.3834 15.5211C12.5289 13.8653 7.23035 15.6536 7.82644 11.9446Z"
                                            fill="#f6f6f6" />
                                        <path
                                            d="M29.683 11.9446C30.1466 9.03034 33.7893 5.91742 36.6374 5.85119C36.8361 5.85119 37.0348 5.78496 37.1673 5.65249C37.2998 5.58626 37.4322 5.52003 37.4985 5.32133C38.492 3.46683 37.9622 2.00972 36.3064 0.817536C34.3856 -0.573341 31.3389 0.817536 29.8155 2.07595C25.974 5.2551 22.0663 10.7524 22.265 15.9847C21.6027 19.4951 21.7351 23.2703 22.7285 26.3832C23.3908 28.3702 25.3116 29.3636 27.2987 29.4961C29.2856 29.6287 33.392 30.2247 35.1803 29.0326C36.9685 27.8403 37.101 25.5222 37.2997 23.5353C37.4984 21.3496 38.0945 17.2431 36.24 15.5211C34.3855 13.8653 29.0207 15.6536 29.683 11.9446Z"
                                            fill="#f6f6f6" />
                                    </svg>
                                </span>
                            </div>
                            <p class="testimonial-details">Es posible crear prácticamente cualquier diseño imaginable y 
                                se permiten personalizaciones a todos los niveles.
                                Algunas funciones podrían beneficiarse de mejores controles.
                                Si sabes manejar el ratón, ya estás listo para utilizar este constructor de páginas.
                            </p>
                            <div class="ratings">
                                <span>
                                    <svg width="75" height="15" viewBox="0 0 75 15" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M22.5 0L24.1839 5.18237H29.6329L25.2245 8.38525L26.9084 13.5676L22.5 10.3647L18.0916 13.5676L19.7755 8.38525L15.3671 5.18237H20.8161L22.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M37.5 0L39.1839 5.18237H44.6329L40.2245 8.38525L41.9084 13.5676L37.5 10.3647L33.0916 13.5676L34.7755 8.38525L30.3671 5.18237H35.8161L37.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M52.5 0L54.1839 5.18237H59.6329L55.2245 8.38525L56.9084 13.5676L52.5 10.3647L48.0916 13.5676L49.7755 8.38525L45.3671 5.18237H50.8161L52.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M67.5 0L69.1839 5.18237H74.6329L70.2245 8.38525L71.9084 13.5676L67.5 10.3647L63.0916 13.5676L64.7755 8.38525L60.3671 5.18237H65.8161L67.5 0Z"
                                            fill="#FFA800" />
                                    </svg>
                                </span>
                            </div>
                            <div class="divider"></div>
                            <div class="testimonial-info">
                                <div class="testimonial-img">
                                    <img src="assets/images/homepage-one/about/testimonial-img-2.webp" alt="img">
                                </div>
                                <div class="testimonial-info-details">
                                    <h5 class="testimonial-name">Yahir Peralta</h5>
                                    <p class="testimonial-title">Director financiero de <span class="title-inner">DesginX</span></p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide testimonial-wrapper">
                            <div class="blockquote">
                                <span>
                                    <svg width="38" height="30" viewBox="0 0 38 30" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.82644 11.9446C8.29006 9.03034 11.9328 5.91742 14.7808 5.85119C14.9795 5.85119 15.1782 5.78496 15.3107 5.65249C15.4431 5.58626 15.5756 5.52003 15.6418 5.32133C16.6353 3.46683 16.1055 2.00972 14.4497 0.817536C12.5289 -0.573341 9.48225 0.817536 7.9589 2.07595C4.11743 5.2551 0.20973 10.7523 0.408427 15.9847C-0.253896 19.4951 -0.121431 23.2703 0.872052 26.3832C1.53437 28.3702 3.45511 29.3636 5.44208 29.4961C7.42905 29.6287 11.5354 30.2247 13.3237 29.0326C15.112 27.8403 15.2445 25.5222 15.4431 23.5353C15.6418 21.3496 16.2379 17.2431 14.3834 15.5211C12.5289 13.8653 7.23035 15.6536 7.82644 11.9446Z"
                                            fill="#f6f6f6" />
                                        <path
                                            d="M29.683 11.9446C30.1466 9.03034 33.7893 5.91742 36.6374 5.85119C36.8361 5.85119 37.0348 5.78496 37.1673 5.65249C37.2998 5.58626 37.4322 5.52003 37.4985 5.32133C38.492 3.46683 37.9622 2.00972 36.3064 0.817536C34.3856 -0.573341 31.3389 0.817536 29.8155 2.07595C25.974 5.2551 22.0663 10.7524 22.265 15.9847C21.6027 19.4951 21.7351 23.2703 22.7285 26.3832C23.3908 28.3702 25.3116 29.3636 27.2987 29.4961C29.2856 29.6287 33.392 30.2247 35.1803 29.0326C36.9685 27.8403 37.101 25.5222 37.2997 23.5353C37.4984 21.3496 38.0945 17.2431 36.24 15.5211C34.3855 13.8653 29.0207 15.6536 29.683 11.9446Z"
                                            fill="#f6f6f6" />
                                    </svg>
                                </span>
                            </div>
                            <p class="testimonial-details">Como agencia de marketing digital, nuestro equipo trabaja día tras día en sitios web de todo tipo.
                                Algunos de los errores más frecuentes que observamos son sitios web no optimizados para SEO debido a temas o diseños antiguos, 
                                monótonos o desactualizados.
                            </p>
                            <div class="ratings">
                                <span>
                                    <svg width="75" height="15" viewBox="0 0 75 15" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M22.5 0L24.1839 5.18237H29.6329L25.2245 8.38525L26.9084 13.5676L22.5 10.3647L18.0916 13.5676L19.7755 8.38525L15.3671 5.18237H20.8161L22.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M37.5 0L39.1839 5.18237H44.6329L40.2245 8.38525L41.9084 13.5676L37.5 10.3647L33.0916 13.5676L34.7755 8.38525L30.3671 5.18237H35.8161L37.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M52.5 0L54.1839 5.18237H59.6329L55.2245 8.38525L56.9084 13.5676L52.5 10.3647L48.0916 13.5676L49.7755 8.38525L45.3671 5.18237H50.8161L52.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M67.5 0L69.1839 5.18237H74.6329L70.2245 8.38525L71.9084 13.5676L67.5 10.3647L63.0916 13.5676L64.7755 8.38525L60.3671 5.18237H65.8161L67.5 0Z"
                                            fill="#FFA800" />
                                    </svg>
                                </span>
                            </div>
                            <div class="divider"></div>
                            <div class="testimonial-info">
                                <div class="testimonial-img">
                                    <img src="assets/images/homepage-one/about/testimonial-img-3.webp" alt="img">
                                </div>
                                <div class="testimonial-info-details">
                                    <h5 class="testimonial-name">Enrique García</h5>
                                    <p class="testimonial-title">Recursos humanos de <span class="title-inner">Desgin360</span></p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide testimonial-wrapper">
                            <div class="blockquote">
                                <span>
                                    <svg width="38" height="30" viewBox="0 0 38 30" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.82644 11.9446C8.29006 9.03034 11.9328 5.91742 14.7808 5.85119C14.9795 5.85119 15.1782 5.78496 15.3107 5.65249C15.4431 5.58626 15.5756 5.52003 15.6418 5.32133C16.6353 3.46683 16.1055 2.00972 14.4497 0.817536C12.5289 -0.573341 9.48225 0.817536 7.9589 2.07595C4.11743 5.2551 0.20973 10.7523 0.408427 15.9847C-0.253896 19.4951 -0.121431 23.2703 0.872052 26.3832C1.53437 28.3702 3.45511 29.3636 5.44208 29.4961C7.42905 29.6287 11.5354 30.2247 13.3237 29.0326C15.112 27.8403 15.2445 25.5222 15.4431 23.5353C15.6418 21.3496 16.2379 17.2431 14.3834 15.5211C12.5289 13.8653 7.23035 15.6536 7.82644 11.9446Z"
                                            fill="#f6f6f6" />
                                        <path
                                            d="M29.683 11.9446C30.1466 9.03034 33.7893 5.91742 36.6374 5.85119C36.8361 5.85119 37.0348 5.78496 37.1673 5.65249C37.2998 5.58626 37.4322 5.52003 37.4985 5.32133C38.492 3.46683 37.9622 2.00972 36.3064 0.817536C34.3856 -0.573341 31.3389 0.817536 29.8155 2.07595C25.974 5.2551 22.0663 10.7524 22.265 15.9847C21.6027 19.4951 21.7351 23.2703 22.7285 26.3832C23.3908 28.3702 25.3116 29.3636 27.2987 29.4961C29.2856 29.6287 33.392 30.2247 35.1803 29.0326C36.9685 27.8403 37.101 25.5222 37.2997 23.5353C37.4984 21.3496 38.0945 17.2431 36.24 15.5211C34.3855 13.8653 29.0207 15.6536 29.683 11.9446Z"
                                            fill="#f6f6f6" />
                                    </svg>
                                </span>
                            </div>
                            <p class="testimonial-details">Es un hecho bien establecido que el contenido legible de una página distrae al lector cuando este observa su diseño. 
                            </p>
                            <div class="ratings">
                                <span>
                                    <svg width="75" height="15" viewBox="0 0 75 15" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M22.5 0L24.1839 5.18237H29.6329L25.2245 8.38525L26.9084 13.5676L22.5 10.3647L18.0916 13.5676L19.7755 8.38525L15.3671 5.18237H20.8161L22.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M37.5 0L39.1839 5.18237H44.6329L40.2245 8.38525L41.9084 13.5676L37.5 10.3647L33.0916 13.5676L34.7755 8.38525L30.3671 5.18237H35.8161L37.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M52.5 0L54.1839 5.18237H59.6329L55.2245 8.38525L56.9084 13.5676L52.5 10.3647L48.0916 13.5676L49.7755 8.38525L45.3671 5.18237H50.8161L52.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M67.5 0L69.1839 5.18237H74.6329L70.2245 8.38525L71.9084 13.5676L67.5 10.3647L63.0916 13.5676L64.7755 8.38525L60.3671 5.18237H65.8161L67.5 0Z"
                                            fill="#FFA800" />
                                    </svg>
                                </span>
                            </div>
                            <div class="divider"></div>
                            <div class="testimonial-info">
                                <div class="testimonial-img">
                                    <img src="assets/images/homepage-one/about/testimonial-img-1.webp" alt="img">
                                </div>
                                <div class="testimonial-info-details">
                                    <h5 class="testimonial-name">Joaquin Martinez</h5>
                                    <p class="testimonial-title">Diseñador de <span class="title-inner">DesginCode</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide testimonial-wrapper">
                            <div class="blockquote">
                                <span>
                                    <svg width="38" height="30" viewBox="0 0 38 30" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.82644 11.9446C8.29006 9.03034 11.9328 5.91742 14.7808 5.85119C14.9795 5.85119 15.1782 5.78496 15.3107 5.65249C15.4431 5.58626 15.5756 5.52003 15.6418 5.32133C16.6353 3.46683 16.1055 2.00972 14.4497 0.817536C12.5289 -0.573341 9.48225 0.817536 7.9589 2.07595C4.11743 5.2551 0.20973 10.7523 0.408427 15.9847C-0.253896 19.4951 -0.121431 23.2703 0.872052 26.3832C1.53437 28.3702 3.45511 29.3636 5.44208 29.4961C7.42905 29.6287 11.5354 30.2247 13.3237 29.0326C15.112 27.8403 15.2445 25.5222 15.4431 23.5353C15.6418 21.3496 16.2379 17.2431 14.3834 15.5211C12.5289 13.8653 7.23035 15.6536 7.82644 11.9446Z"
                                            fill="#f6f6f6" />
                                        <path
                                            d="M29.683 11.9446C30.1466 9.03034 33.7893 5.91742 36.6374 5.85119C36.8361 5.85119 37.0348 5.78496 37.1673 5.65249C37.2998 5.58626 37.4322 5.52003 37.4985 5.32133C38.492 3.46683 37.9622 2.00972 36.3064 0.817536C34.3856 -0.573341 31.3389 0.817536 29.8155 2.07595C25.974 5.2551 22.0663 10.7524 22.265 15.9847C21.6027 19.4951 21.7351 23.2703 22.7285 26.3832C23.3908 28.3702 25.3116 29.3636 27.2987 29.4961C29.2856 29.6287 33.392 30.2247 35.1803 29.0326C36.9685 27.8403 37.101 25.5222 37.2997 23.5353C37.4984 21.3496 38.0945 17.2431 36.24 15.5211C34.3855 13.8653 29.0207 15.6536 29.683 11.9446Z"
                                            fill="#f6f6f6" />
                                    </svg>
                                </span>
                            </div>
                            <p class="testimonial-details">Crea la tienda online perfecta con nuestra plantilla de 
                                sitio web tipo «Brandstore» de alta conversión.
                            </p>
                            <div class="ratings">
                                <span>
                                    <svg width="75" height="15" viewBox="0 0 75 15" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M22.5 0L24.1839 5.18237H29.6329L25.2245 8.38525L26.9084 13.5676L22.5 10.3647L18.0916 13.5676L19.7755 8.38525L15.3671 5.18237H20.8161L22.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M37.5 0L39.1839 5.18237H44.6329L40.2245 8.38525L41.9084 13.5676L37.5 10.3647L33.0916 13.5676L34.7755 8.38525L30.3671 5.18237H35.8161L37.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M52.5 0L54.1839 5.18237H59.6329L55.2245 8.38525L56.9084 13.5676L52.5 10.3647L48.0916 13.5676L49.7755 8.38525L45.3671 5.18237H50.8161L52.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M67.5 0L69.1839 5.18237H74.6329L70.2245 8.38525L71.9084 13.5676L67.5 10.3647L63.0916 13.5676L64.7755 8.38525L60.3671 5.18237H65.8161L67.5 0Z"
                                            fill="#FFA800" />
                                    </svg>
                                </span>
                            </div>
                            <div class="divider"></div>
                            <div class="testimonial-info">
                                <div class="testimonial-img">
                                    <img src="assets/images/homepage-one/about/testimonial-img-1.webp" alt="img">
                                </div>
                                <div class="testimonial-info-details">
                                    <h5 class="testimonial-name">Vicente Herrera</h5>
                                    <p class="testimonial-title">Director ejecutivo de <span class="title-inner">DesginLab</span></p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide testimonial-wrapper">
                            <div class="blockquote">
                                <span>
                                    <svg width="38" height="30" viewBox="0 0 38 30" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.82644 11.9446C8.29006 9.03034 11.9328 5.91742 14.7808 5.85119C14.9795 5.85119 15.1782 5.78496 15.3107 5.65249C15.4431 5.58626 15.5756 5.52003 15.6418 5.32133C16.6353 3.46683 16.1055 2.00972 14.4497 0.817536C12.5289 -0.573341 9.48225 0.817536 7.9589 2.07595C4.11743 5.2551 0.20973 10.7523 0.408427 15.9847C-0.253896 19.4951 -0.121431 23.2703 0.872052 26.3832C1.53437 28.3702 3.45511 29.3636 5.44208 29.4961C7.42905 29.6287 11.5354 30.2247 13.3237 29.0326C15.112 27.8403 15.2445 25.5222 15.4431 23.5353C15.6418 21.3496 16.2379 17.2431 14.3834 15.5211C12.5289 13.8653 7.23035 15.6536 7.82644 11.9446Z"
                                            fill="#f6f6f6" />
                                        <path
                                            d="M29.683 11.9446C30.1466 9.03034 33.7893 5.91742 36.6374 5.85119C36.8361 5.85119 37.0348 5.78496 37.1673 5.65249C37.2998 5.58626 37.4322 5.52003 37.4985 5.32133C38.492 3.46683 37.9622 2.00972 36.3064 0.817536C34.3856 -0.573341 31.3389 0.817536 29.8155 2.07595C25.974 5.2551 22.0663 10.7524 22.265 15.9847C21.6027 19.4951 21.7351 23.2703 22.7285 26.3832C23.3908 28.3702 25.3116 29.3636 27.2987 29.4961C29.2856 29.6287 33.392 30.2247 35.1803 29.0326C36.9685 27.8403 37.101 25.5222 37.2997 23.5353C37.4984 21.3496 38.0945 17.2431 36.24 15.5211C34.3855 13.8653 29.0207 15.6536 29.683 11.9446Z"
                                            fill="#f6f6f6" />
                                    </svg>
                                </span>
                            </div>
                            <p class="testimonial-details">Las plantillas de sitios web de comercio electrónico, 
                                ligeras y totalmente adaptables, 
                                están diseñadas para ofrecer velocidad y favorecer la conversión, 
                                ayudándole a vender más con poco esfuerzo adicional.
                            </p>
                            <div class="ratings">
                                <span>
                                    <svg width="75" height="15" viewBox="0 0 75 15" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M22.5 0L24.1839 5.18237H29.6329L25.2245 8.38525L26.9084 13.5676L22.5 10.3647L18.0916 13.5676L19.7755 8.38525L15.3671 5.18237H20.8161L22.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M37.5 0L39.1839 5.18237H44.6329L40.2245 8.38525L41.9084 13.5676L37.5 10.3647L33.0916 13.5676L34.7755 8.38525L30.3671 5.18237H35.8161L37.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M52.5 0L54.1839 5.18237H59.6329L55.2245 8.38525L56.9084 13.5676L52.5 10.3647L48.0916 13.5676L49.7755 8.38525L45.3671 5.18237H50.8161L52.5 0Z"
                                            fill="#FFA800" />
                                        <path
                                            d="M67.5 0L69.1839 5.18237H74.6329L70.2245 8.38525L71.9084 13.5676L67.5 10.3647L63.0916 13.5676L64.7755 8.38525L60.3671 5.18237H65.8161L67.5 0Z"
                                            fill="#FFA800" />
                                    </svg>
                                </span>
                            </div>
                            <div class="divider"></div>
                            <div class="testimonial-info">
                                <div class="testimonial-img">
                                    <img src="assets/images/homepage-one/about/testimonial-img-1.webp" alt="img">
                                </div>
                                <div class="testimonial-info-details">
                                    <h5 class="testimonial-name">Santiago López</h5>
                                    <p class="testimonial-title">Desarrollador de <span class="title-inner">DesginUX</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-pagination"></div>
            </div>
        </div>

    </section>

    <!--------------- about-slider-section-end---------------->

    <!--------------- latest-news-section---------------->
    <!--------------- latest-news-section---------------->
    <section class="latest product footer-padding">
        <div class="container">
            <div class="section-title text-center">
                <h4 class="about-details">Mis últimas noticias</h4>
            </div>
            <div class="latest-section">
                <div class="row g-5">


                    <div class="col-lg-4 col-sm-6">
                        <div class="blogs-wrapper product-wrapper" data-aos="fade-up" data-aos-duration="300">
                            <div class="wrapper-img">
                                <img src="{{ asset('https://scontent.fisj3-3.fna.fbcdn.net/v/t39.30808-6/658378315_122101274246950755_1244296750272920169_n.jpg?stp=dst-jpg_tt6&cstp=mx1600x1064&ctp=s590x590&_nc_cat=100&_nc_map=urlgen_bucketless&ccb=1-7&_nc_sid=833d8c&_nc_eui2=AeFXieJvoJ-nLOOwOuk0dI21SDlb-ppd2IBIOVv6ml3YgLwzKbt7VWltwz0QBBraBC_QkwlbxsQUaBQnQflZarKT&_nc_ohc=uayo7rsLvqYQ7kNvwHWmNWq&_nc_oc=AdrAV_ZyB_WkwQ2DoFCJ_dsN2PhPCi2hO8Dak379dAkLKnl6kEAONA8LZ_NyDKs1z3I&_nc_zt=23&_nc_ht=scontent.fisj3-3.fna&_nc_gid=gK27wcNd3TksEDbcD8yGEw&_nc_ss=7b2a8&oh=00_AQLGl0QYFQTTkC2SMys1d7D-GhesICj1YqackJdAri0JrA&oe=6AB9CABD') }}" alt="Sublimación de Tazas y Playeras">
                            </div>
                            <div class="wrapper-info">
                                <div class="wrapper-data">
                                    <div class="admin wrapper-item">
                                        <span class="icon">
                                            <svg width="12" height="15" viewBox="0 0 12 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M1.761 14.9996C1.55973 14.9336 1.35152 14.8896 1.16065 14.7978C0.397206 14.4272 -0.02963 13.6273 0.00160193 12.743C0.0397743 11.6936 0.275749 10.7103 0.765049 9.7966C1.42439 8.56373 2.36829 7.65741 3.59327 7.07767C3.67309 7.04098 3.7529 7.00428 3.85007 6.95658C2.68061 5.9512 2.17396 4.67062 2.43422 3.10017C2.58691 2.18285 3.03804 1.42698 3.72514 0.847238C5.24163 -0.42967 7.34458 -0.216852 8.60773 1.1738C9.36424 2.00673 9.70779 3.01211 9.61757 4.16426C9.52734 5.31642 9.01375 6.23374 8.14619 6.94924C8.33359 7.04098 8.50363 7.11436 8.6702 7.20609C10.1485 8.006 11.1618 9.24254 11.6997 10.9011C11.9253 11.5945 12.0328 12.3137 11.9912 13.0476C11.9357 14.0163 11.2243 14.8235 10.3151 14.9703C10.2908 14.974 10.2665 14.9886 10.2387 14.9996Z" fill="#AE1C9A" />
                                            </svg>
                                        </span>
                                        <span class="text">DOPE PRINTS</span>
                                    </div>
                                </div>
                                <a href="/blogs-details" class="about-details wrapper-details">
                                    Nuevos diseños y acabados en sublimación de tazas y playeras personalizados
                                </a>
                                <div class="divider"></div>
                                <a href="/contact-us" class="shop-btn">
                                    Saber más
                                    <span>
                                        <svg width="16" height="11" viewBox="0 0 16 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M12.6227 4.38176C12.5587 4.38176 12.4989 4.38176 12.4349 4.38176C8.56302 4.38176 4.69114 4.38176 0.819254 4.38176C0.7168 4.38176 0.614347 4.37785 0.516163 4.40129C0.195996 4.4677 -0.0302552 4.76459 0.00389589 5.05758C0.0423159 5.37791 0.302718 5.60839 0.644229 5.62793C0.712532 5.63183 0.780834 5.63183 0.853405 5.63183Z" fill="#AE1C9A" />
                                        </svg>
                                    </span>
                                </a>
                            </div>
                        </div>
                    </div>


                    <div class="col-lg-4 col-sm-6">
                        <div class="blogs-wrapper product-wrapper" data-aos="fade-up" data-aos-duration="400">
                            <div class="wrapper-img">
                                <img src="{{ asset('assets/img/about/cursos_robotica.jpeg') }}" alt="Cursos de Robótica y Fotografía">
                            </div>
                            <div class="wrapper-info">
                                <div class="wrapper-data">
                                    <div class="admin wrapper-item">
                                        <span class="icon">
                                            <svg width="12" height="15" viewBox="0 0 12 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M1.761 14.9996C1.55973 14.9336 1.35152 14.8896 1.16065 14.7978C0.397206 14.4272 -0.02963 13.6273 0.00160193 12.743C0.0397743 11.6936 0.275749 10.7103 0.765049 9.7966C1.42439 8.56373 2.36829 7.65741 3.59327 7.07767C3.67309 7.04098 3.7529 7.00428 3.85007 6.95658C2.68061 5.9512 2.17396 4.67062 2.43422 3.10017C2.58691 2.18285 3.03804 1.42698 3.72514 0.847238C5.24163 -0.42967 7.34458 -0.216852 8.60773 1.1738C9.36424 2.00673 9.70779 3.01211 9.61757 4.16426C9.52734 5.31642 9.01375 6.23374 8.14619 6.94924C8.33359 7.04098 8.50363 7.11436 8.6702 7.20609C10.1485 8.006 11.1618 9.24254 11.6997 10.9011C11.9253 11.5945 12.0328 12.3137 11.9912 13.0476C11.9357 14.0163 11.2243 14.8235 10.3151 14.9703Z" fill="#AE1C9A" />
                                            </svg>
                                        </span>
                                        <span class="text"></span>
                                    </div>
                                </div>
                                <a href="/blogs-details" class="about-details wrapper-details">
                                    Inauguramos nuevos talleres y cursos especializados en robótica
                                </a>
                                <div class="divider"></div>
                                <a href="/contact-us" class="shop-btn">
                                    Saber más
                                    <span>
                                        <svg width="16" height="11" viewBox="0 0 16 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M12.6227 4.38176C12.5587 4.38176 12.4989 4.38176 12.4349 4.38176C8.56302 4.38176 4.69114 4.38176 0.819254 4.38176C0.7168 4.38176 0.614347 4.37785 0.516163 4.40129C0.195996 4.4677 -0.0302552 4.76459 0.00389589 5.05758C0.0423159 5.37791 0.302718 5.60839 0.644229 5.62793Z" fill="#AE1C9A" />
                                        </svg>
                                    </span>
                                </a>
                            </div>
                        </div>
                    </div>


                    <div class="col-lg-4 col-sm-6">
                        <div class="blogs-wrapper product-wrapper" data-aos="fade-up" data-aos-duration="600">
                            <div class="wrapper-img">
                                <img src="{{ asset('assets/img/about/cut.jpeg') }}" alt="Corte Láser e Impresión 3D">
                            </div>
                            <div class="wrapper-info">
                                <div class="wrapper-data">
                                    <div class="admin wrapper-item">
                                        <span class="icon">
                                            <svg width="12" height="15" viewBox="0 0 12 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M1.761 14.9996C1.55973 14.9336 1.35152 14.8896 1.16065 14.7978C0.397206 14.4272 -0.02963 13.6273 0.00160193 12.743C0.0397743 11.6936 0.275749 10.7103 0.765049 9.7966C1.42439 8.56373 2.36829 7.65741 3.59327 7.07767C3.67309 7.04098 3.7529 7.00428 3.85007 6.95658C2.68061 5.9512 2.17396 4.67062 2.43422 3.10017C2.58691 2.18285 3.03804 1.42698 3.72514 0.847238Z" fill="#AE1C9A" />
                                            </svg>
                                        </span>
                                        <span class="text">Innova-cut</span>
                                    </div>
                                </div>
                                <a href="/blogs-details" class="about-details wrapper-details">
                                    Nuevos servicios de prototipado rápido concorte láser
                                </a>
                                <div class="divider"></div>
                                <a href="/contact-us" class="shop-btn">
                                    Saber más
                                    <span>
                                        <svg width="16" height="11" viewBox="0 0 16 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M12.6227 4.38176C12.5587 4.38176 12.4989 4.38176 12.4349 4.38176C8.56302 4.38176 4.69114 4.38176 0.819254 4.38176C0.7168 4.38176 0.614347 4.37785 0.516163 4.40129Z" fill="#AE1C9A" />
                                        </svg>
                                    </span>
                                </a>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </section>
    <!--------------- latest-news-section-end---------------->
@endsection
